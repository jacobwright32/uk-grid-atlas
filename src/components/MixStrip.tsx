 ...
 */
export default function MixStrip({ mix, rows, mode, title }: Props) {
  ...
  const subtitle =
    mode === 'daily'
      ...
      : mode === 'snapshot'
        ? 'snapshot'
        : when.toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit', timeZone: 'Europe/London' })
    ...
    <div className="mixstrip" role="figure" aria-label="Current GB generation vs fleet capacity">
      ...
      <div className="fleet-rows">
        {rows.map((r) => {
          ...
          const fillPct = r.capMW > 0 ? Math.min(100, (100 * r.nowMW) / Math.max(r.capMW, r.nowMW)) : 100
          ...
          return (
            ...
            >
              ...
              <span className="fleet-bar">
                <span className="fleet-track" style={{ width: `${trackPct}%`, background: r.color }}>
                  <span className="fleet-fill" style={{ width: `${fillPct}%`, background: r.color }} />
                </span>
              </span>
      ...
      <div className="mixstrip-legend">
        ...
        <span className="mixstrip-note">
          {mode === 'daily' ? 'transmission-metered generation' : 'solar & embedded not metered here'}
        </span>
      </div>
    </div>
  )
