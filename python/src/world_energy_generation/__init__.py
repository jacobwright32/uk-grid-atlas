...
for 25 power grids across Europe and North America, as typed Python.
...
``history(code)``
    ...
    resolution. Available for all 25 grids. This is the uniform surface — reach
    for it first.

``live(code)``
    The most recent snapshot. Available for 24 grids; Great Britain's is
...
def history(code: str, *, client: Client | None = None) -> History:
    """
    Rolling ~31-day history for one grid. Available for all 25.
...
def live(code: str, *, client: Client | None = None) -> LiveSnapshot:
    """
    Most recent snapshot for one grid. Available for 24 of 25.
