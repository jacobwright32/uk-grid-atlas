...
for 22 power grids across Europe and North America, as typed Python.
...
``history(code)``
    ...
    resolution. Available for all 22 grids. This is the uniform surface — reach
    for it first.

``live(code)``
    The most recent snapshot. Available for 21 grids; Great Britain's is
...
def live(code: str, *, client: Client | None = None) -> LiveSnapshot:
    """
    Most recent snapshot for one grid. Available for 21 of 22.
