...
Electricity generation, prices, cross-border flows and demand for **28 power grids** across Europe and North America, as typed Python. No API key, no account, no required dependencies.
...
### `history(code)` — rolling ~31 days, all 28 grids
...
### `live(code)` — most recent snapshot, 27 grids
...
| `pt` `es` `fr` `be` `nl` `de` `ch` `it` `at` `cz` `si` `hu` `sk` `hr` `bg` `gr` `pl` | Portugal, Spain, France, Belgium, Netherlands, Germany, Switzerland, Italy, Austria, Czechia, Slovenia, Hungary, Slovakia, Croatia, Bulgaria, Greece, Poland | ✓ | ENTSO-E |
...
weg.grids(live_only=True)                # the 27 with live JSON
...
weg.demand()                      # all 28: grid, date, demand_mw
```
