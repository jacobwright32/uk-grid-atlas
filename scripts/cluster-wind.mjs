 ...
 * ~20 MW — divide by 1000 until plausible. Decimal-comma mode: turbine
 * ratings come from continental OSM ("2,3 MW" = 2.3).
 */
export function turbineMW(tag) {
  // parseCapacityMW's 2nd param is a boolean — passing `{decimalComma:true}`
  // (truthy) happened to behave, but `(tag, true)` is what's meant (#48).
  let mw = parseCapacityMW(tag, true)
  if (mw == null) return null
  while (mw > 20) mw /= 1000
  return mw >= 0.05 ? mw : null
}

/** "Kooninkulma 4" → "Kooninkulma" · "Mäkikangas WTG 7" → "Mäkikangas". */
export const stripUnit = (name) =>
  name
